...
import { GET_BELI_SAMPAH_FASILITATOR } from '../../api/sampah';
import { fDateTime } from '../../utils/formatTime';

export default function TransaksiPembelian({parammitra}) {
  console.log(parammitra);
  const [page, setPage] = useState(0);
  const [loading, setLoading] = useState(0);
  const [size, setSize] = useState(5);
  const [date, setDate] = useState('');
  const [mitraparemdata, setMitraparamdata] = useState(parammitra);
  const [detailOpen, setDetailOpen] = useState(null);
  const { data, refetch, isLoading } = useQuery('GET_BELI_SAMPAH', () => GET_BELI_SAMPAH_FASILITATOR(mitraparemdata,{ page, size, date }), {
  ...
  useEffect(() => {
    setDate('');
    setLoading(true);
    refetch({ mitraparemdata,page, size, date });
    setLoading(false);
