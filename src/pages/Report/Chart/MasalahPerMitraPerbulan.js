...
import { GET_MASALAH_PERBULAN_JENIS_STATUS } from '../../../api/report';
...
export default function MasalahPerMitraPerbulan() {
  ...
  const { data, isLoading, refetch } = useQuery(['GET_SEMUA_MASALAH_PERBULAN_JENIS_STATUS', tahun?.value], () =>
    GET_MASALAH_PERBULAN_JENIS_STATUS(tahun?.value, jenisMasalah?.value, mitraCode?.value, status?.value)
  );
