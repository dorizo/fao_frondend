...
import { GET_ANALISI_PEMBELIAN_MITRA_PERBULAN } from '../../../api/report';
import AutoCompleteLoading from '../../../components/AutoCompleteLoading';
import { yearOption } from '../../../utils/yearOption';

const tahunSekarang = new Date().getFullYear();

export default function PembelianPerMitra() {
  ...
  const { data, isLoading, refetch } = useQuery(['GET_ANALISI_PEMBELIAN_MITRA_PERBULAN', tahun?.value], () =>
    GET_ANALISI_PEMBELIAN_MITRA_PERBULAN(tahun?.value)
  );
  const list = data && data?.data?.data;
  const chartDataMitra = [...Array(12)].map(() => 0);
  const chartDataBerat = [...Array(12)].map(() => 0);
  const chartDataJumlah = [...Array(12)].map(() => 0);
  if (!isLoading) {
    list?.forEach((v) => {
      chartDataMitra[v.bulan - 1] = v.jumlahMitra;
      chartDataJumlah[v.bulan - 1] = v.jumlahBeli;
      chartDataBerat[v.bulan - 1] = v.totalBerat;
    });
  }
  const state = {
    series: [
    ...
    options: {
      ...
      title: {
        text: 'Analisis Pembelian per mitra',
        align: 'left',
      },
      grid: {
        row: {
